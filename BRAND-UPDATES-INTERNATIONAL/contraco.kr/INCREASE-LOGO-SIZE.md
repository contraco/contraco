# TASK: Increase Logo Size from 40px to 58px - contraco.kr
...
Increase logo size from **40px to 58px** across all contraco.kr HTML files to match contraco.net standard.
...
git commit -m "Increase logo size from 40px to 58px across contraco.kr
...
Result: All 15 contraco.kr HTML files now have uniform 58px logo"
```
...
1. **Preserve Korean text (한글)** in navigation and content - do NOT modify
2. **Match exact indentation** from the file (usually 8 or 12 spaces)
3. **Some files may have `transition: opacity`** - optionally change to `transition: transform`
4. **Korean typography:** Files use Inter font (acceptable) - Noto Sans KR would be better but not required for this task
5. **Do NOT modify** yandex verification files or css_base_styles.html
...
```
Task: Increase logo size from 40px to 58px on contraco.kr

Read: BRAND-UPDATES-INTERNATIONAL/contraco.kr/INCREASE-LOGO-SIZE.md
...
cultural-adaptation-guide.html, leading-ai-transformation.html,
organizational-design-ai.html, pricing-psychology.html,
resonance-method.html, strategic-intelligence.html

Preserve Korean text! Use Edit tool for each file. Validate. Commit.
```
